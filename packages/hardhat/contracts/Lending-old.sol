...
contract Sender is CCIPReceiver, OwnerIsCreator {
  ...
  event Depositing(
    address depositor, // The address of the depositor from the source chain.
    address token,
    uint tokenAmount // The token amount that was sent.
  );

  event Unstake (
    address depositor, // The address of the depositor from the source chain.
    uint tokenAmount // The token amount that was unstaked.
  );
  ...
  mapping(address => mapping(address => TokenInfo)) public addressToTokens;
  ...
  function sendMessage(
    ...
    Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
      ...
      extraArgs: Client._argsToBytes(
        Client.EVMExtraArgsV1({gasLimit: 980_000}) // Additional arguments, setting gas limit and non-strict sequency mode
      ),
  ...
  function deposit(address token, uint amount) external payable {
    ...
    // Record Deposit on user balance
    recordDeposit(msg.sender, token, amount);
    // Emit an event indicating the successful deposit
    emit Depositing(msg.sender, token, amount);
  }

  function recordDeposit(address sender, address token, uint256 amount) internal {
  ...
  function unstake(address token, uint256 amount) public {
    ...
    // Record Unstaking
    recordUnstake(msg.sender, token, amount);
    // Emit event
    emit Unstake(msg.sender, amount);
  }

  function recordUnstake(address sender, address token, uint256 amount) internal {
    addressToTokens[sender][token].amount -= amount;
  }
